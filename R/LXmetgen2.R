...
#------------------Gene enriched pathways analysis-----------------------------#
 ...
 #---kegg pathways enrichment--------
    ...
    kegg_all_pathways <- pathways_geneSymbol
 ...
 #--------------------------------------------
  ...
  #------------kegg metabolism pathways----------------#
    #筛选出含有"METABOL"字段的行
    kegg_meta_all <- dplyr::filter(kegg_all_pathways, category=="Metabolism")

    if(any(kegg_meta_all$Description=="Metabolic pathways"))
      kegg_meta_all <- kegg_meta_all[-grep("Metabolic pathways",kegg_meta_all$Description,ignore.case = T),]
...
nn <- c(grep("Description",colnames(kegg_meta_all),ignore.case = T),
        grep("pvalue",colnames(kegg_meta_all),ignore.case = T))

kegg_meta <- kegg_meta_all[,nn]
